...
const migrations = (await readdir(new URL('../server/d1/migrations/', import.meta.url))).filter((name) => /^000[1-7]_/.test(name)).sort()
if (migrations.join(',') !== '0001_initial_schema.sql,0002_auth_identity_foundation.sql,0003_user_data_ownership.sql,0004_continuous_user_sync.sql,0005_admin_foundation.sql,0006_catalog_foundation.sql,0007_catalog_packages_variants.sql') throw new Error('D1 migration sırası geçersiz.')
