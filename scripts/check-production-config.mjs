...
const migrations = (await readdir(new URL('../server/d1/migrations/', import.meta.url))).filter((name) => /^000[2-5]_/.test(name)).sort()
if (migrations.join(',') !== '0002_auth_identity_foundation.sql,0003_user_data_ownership.sql,0004_continuous_user_sync.sql,0005_admin_foundation.sql') throw new Error('Auth/sync migration sırası geçersiz.')
