...
- **Veri genişletme (4. revizyon, `catalog:8e3f55e152dc56b0046886f4`)**: kullanıcı talebi — mevcut markalara, özellikle az modelli olanlara (Volvo 3→6, Suzuki 2→4 vb.) yeni model ve bağlı nesil/motor/şanzıman ekle, veri setini ~2 katına çıkar. 27 markaya toplam 68 yeni model eklendi (1-2 modelli markalar iki/üçe katlandı: Alfa Romeo, SsangYong, Mini, Jeep, Citroën, Mazda, Suzuki, Mitsubishi, Land Rover, Jaguar, Subaru, Chevrolet, Tofaş dahil); her model gerçek nesil aralığı ve mevcut motor/şanzıman havuzundan referanslarla geldi. Yalnızca gerçekten eksik olan 7 motor varyantı yeni eklendi (SsangYong 1.6 GDI, Jeep Pentastar 3.6 V6, Jeep 2.8 CRD, JLR 3.0 V6 dizel, JLR Ingenium P300 benzinli, Chevrolet Spark 1.0/1.2, Alfa Romeo 2.2 JTDm). Marka-model sayısı 130 → 198, nesil sayısı 274. Sunucu tarafı doğrulama yine 0 blocking error, 753 uyarı ile geçti ve yayınlandı; önceki revizyon arşivlendi.
- `GET /catalog/meta` artık: 31 marka, 198 model, 274 nesil, 118 motor, 28 şanzıman, 134 paket, 53 donanım, 99 araç varyantı, 20 sorun arketipi, 740 sorun-kapsam eşleşmesi, 14 bakım kalemi, 71 referans değer, 8 değerleme faktörü.
- **Veri genişletme (5. revizyon, `catalog:4a55f882dc53876392349b42`)**: kullanıcı talebi — "hepsinden devasa veriler ekle" (marka, model, motor, şanzıman, kronik sorun, donanım, arıza kodu). Bu tur katalogun her boyutuna dokundu: 4 yeni marka (Lexus, Porsche, MG, DS — `BRAND_GROUPS`'a da eklendi), 35 yeni model (9'u yeni markalara, 26'sı mevcut markalara ikinci/üçüncü kademe modeller — Kia Rio eklenmesi paketlerdeki önceden çözülmemiş bir referansı da düzeltti), 3 yeni motor (Porsche 3.0 V6 Turbo, MG 1.5 Turbo, Ford Ranger 3.2 TDCi — geri kalanı mevcut havuzdan), 10 yeni sistemik kronik arıza arketipi (`problems.js`), 8 yeni donanım kalemi (`equipment.js`), 23 yeni OBD-II arıza kodu (`src/data/obd-codes.json`, katalog D1 hattının dışında ayrı statik dosya). Paket verisi (`packages.js`) bilinçli olarak genişletilmedi — dosyanın kendi başlığı bunu projedeki en kırılgan/hataya açık veri türü olarak tanımlıyor. Marka 31→35, model 198→233, nesil 309, motor 121, sorun arketipi 20→30, donanım 53→61. Sunucu tarafı doğrulama yine 0 blocking error, 753 uyarı ile geçti ve yayınlandı.
- `GET /catalog/meta` artık: 35 marka, 233 model, 309 nesil, 121 motor, 28 şanzıman, 134 paket, 61 donanım, 103 araç varyantı, 30 sorun arketipi, 740 sorun-kapsam eşleşmesi, 14 bakım kalemi, 75 referans değer, 8 değerleme faktörü.
