...
export default {
  async fetch(request, env) {
    ...
    if (url.pathname.startsWith('/admin')) {
      ...
      if (announcement) { const [, id, action] = announcement; const current = await env.DB.prepare('SELECT status FROM announcements WHERE id=?1').bind(id).first(); if (!current) return json({ error: 'Duyuru bulunamadı' }, 404, cors); if (request.method === 'PUT' && !action) { const body = await request.json().catch(() => null); if (!textSafe(body?.title, 120) || !textSafe(body?.message, 2000)) return json({ error: 'Geçersiz duyuru' }, 400, cors); const now=Date.now(); await env.DB.prepare('UPDATE announcements SET title=?1,message=?2,starts_at=?3,ends_at=?4,version=version+1,updated_at=?5,updated_by=?6 WHERE id=?7').bind(body.title.trim(),body.message.trim(),Number(body.starts_at)||null,Number(body.ends_at)||null,now,actor.userId,id).run(); await audit(env,actor,'announcement.update','announcement',id); return json({ ok:true },200,{...cors,'Cache-Control':'no-store'}) } if (request.method === 'POST' && action) { const next = action === 'publish' ? 'published' : action === 'archive' ? 'archived' : 'draft'; if (current.status !== next) { await env.DB.prepare('UPDATE announcements SET status=?1,version=version+1,updated_at=?2,updated_by=?3 WHERE id=?4').bind(next,Date.now(),actor.userId,id).run(); await audit(env,actor,`announcement.${action}`,'announcement',id) } return json({ id,status:next },200,{...cors,'Cache-Control':'no-store'}) } }
      ...
      if (url.pathname === '/admin/audit' && request.method === 'GET') { const limit=Math.min(100,Math.max(1,Number(url.searchParams.get('limit'))||50)); const rows=await env.DB.prepare('SELECT id,actor_user_id,action,target_type,target_id,created_at FROM admin_audit_logs ORDER BY created_at DESC,id DESC LIMIT ?1').bind(limit).all(); return json({ items:rows.results||[] },200,{...cors,'Cache-Control':'no-store'}) }
