...
15. **Boya / Değişen Kontrolü** (`/boya-degisen`, `src/services/paintDetectionService.js`, `src/services/colorScience.js`) — aracın 13 kaporta panelinden ayrı fotoğraf alıp renk ve dokuyu ölçerek birbiriyle karşılaştırır. Ölçüm zincirinin tamamı, ışık gürültüsünü boya sinyalinden ayırmak üzere kurulmuştur:

    - **CIELAB + CIEDE2000.** Renk farkı, endüstride boya eşleştirmesi için kullanılan standartla hesaplanır. HSL kullanılmaz: HSL'de "ton" değeri beyaz/gri/siyah araçlarda gürültüdür ve Türkiye pazarının yarısından fazlası bu renklerdedir.
    - **Pozlama normalizasyonu.** Aynı boyanın güneşteki ve gölgedeki hâli, doğrusal ışık uzayında birbirinin katıdır. Paneller karşılaştırılmadan önce ortak bir parlaklığa ölçeklenir; ayrıca fark hesabında ışık ekseni (ΔL) ile boya ekseni (ΔC, ΔH) ayrılır ve karar yalnızca boya eksenine dayandırılır. Testte, sadece gölgede çekildiği için 8 L\* birim koyu çıkan orijinal bir panelin renk sapması 0,31 ölçüldü — eşiğin çok altında, yani doğru biçimde temiz sayıldı.
    - **Çoklu yama örneklemesi ve eleme.** Panelin tek ortalama rengi alınmaz; 36 yamaya bölünür, parlama/gölge yamaları panelin **kendi** parlaklık dağılımına göre (mutlak eşikle değil) elenir, doymuş pikseller renk hesabına katılmaz, kenar enerjisi yüksek yamalar (fitil, kulp, cam, plaka, arka plan) atılır ve kalanların ortancası kullanılır.
    - **Konsensüs kümesi.** "Normal" panel, ortalamayla değil birbiriyle uyuşan panellerin oluşturduğu en büyük kümeyle belirlenir; böylece panellerin bir kısmı boyalıyken bile referans bozulmaz.
    - **Uyarlamalı eşik.** Konsensüs içindeki panellerin kendi aralarındaki fark, o çekim seansının gürültü tabanı olarak ölçülür ve karar eşiği bunun üzerine kurulur. Telefonun otomatik beyaz dengesi gürültüyü artırırsa eşik kendiliğinden yükselir.
    - **Simetrik ikiz karşılaştırması.** Sol/sağ aynı panel (çamurluklar, kapılar) ayrıca birbiriyle kıyaslanır. Aynı kalıptan çıkmış, aynı eğrilikteki iki panelin karşılaştırması, kaputu kapıyla kıyaslamaktan çok daha güvenilirdir.
    - **Bölgesel tutarlılık.** Bitişik panellerin birlikte sapması kanıtı güçlendirir ve genel yargıyı sertleştirir; gerçek onarım bölgeseldir, ölçüm gürültüsü ise dağınıktır.
    - **Plastik ayrımı.** Tamponlar fabrikada ayrı boyanır ve sacdan farklı çıkmaları normaldir; konsensüse katılmaz, daha toleranslı değerlendirilirler.
    - **Kalite kapısı.** Bulanık veya parlama baskın fotoğraflar puanlanmaz, "ölçülemedi" olarak işaretlenir ve kullanıcıdan tekrar çekmesi istenir. Kötü veriyle cevap üretmek yerine cevap üretmemek tercih edilir.

    Çıktı ikili bir damga değil, panel başına **0-100 kanıt puanı + güven yüzdesi + gerekçe listesi**dir; istenirse ham ölçüm sayıları da gösterilir.

    **Doğrulama:** Motor, bilinen doğru cevapları olan yedi sentetik senaryoyla test edildi — renkli/beyaz/siyah araçta boyalı panel, sadece gölgede çekilmiş orijinal panel, ışığı %20 oynatılmış tamamen orijinal araç (yanlış pozitif testi), parlamalı fotoğraf, plastik tampon ve tek taraflı bölgesel boya. Altısında beklenen sonuç alındı, yanlış pozitif üretilmedi. Yedinci senaryo tespit sınırını gösteriyor: renk kayması 0,3 ΔE altında kalan **çok iyi yapılmış bir boya bu yöntemle görünmüyor.**

    **Sınır (önemli):** Bu ölçüm boya kalınlığı vermez ve mikron cihazının yerini tutmaz; panel çizgisi, aralık geometrisi veya overspray analizi yapmaz. "Fark bulunamadı" sonucu boyasızlık garantisi değildir. Çıktı, "hangi panele mikronla öncelik vermeliyim" sorusunun cevabıdır.
